...
class APIcontroller:
    ...
    def change(self, sentence):
        ...
        types = ['简体', '繁体', '火星文']
        for i in range(3):
            url = 'http://japi.juhe.cn/charconvert/change.from?text=' + parse.quote(sentence) + '&type=' + str(i) + \
                  '&key=' + self.keys[1]
            response = request.Request(url, headers=self.headers)
            obj = json.loads(request.urlopen(response).read().decode('utf-8'))
            if obj['error_code'] == 0:
                text = text + types[i] + '：' + obj['outstr']
            if i != 2:
                text = text + '\n'
        if text != '':
            return text
    ...
    def weather(self, city):
        with open("citycode.csv", 'r', encoding='UTF-8') as file:
